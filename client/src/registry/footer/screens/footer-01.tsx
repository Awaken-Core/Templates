...
import { CtaButton } from "@/components/modules/dashboard/cta-button";
...
const Footer01 = () => {
  ...
  const primaryColor = '#00EB6D';

  return (
    <footer
      className="w-full text-[#17233c] flex flex-col items-center justify-center gap-28"
    ...
    >
        ...
        <div className="grid gap-12 md:grid-cols-[3fr_repeat(4,minmax(130px,1fr))] md:gap-10">
          <div>
            <div className="flex items-center gap-1 cursor-pointer">
              <AppLogo height={44} weight={44} color={primaryColor} />
              <span className="text-[32px] font-bold tracking-[-1.2px] text-[#031129]">
                Awaken Studios
              </span>
            </div>
            <div className="mt-9 max-w-[280px] text-[20px] font-medium leading-[1.45] tracking-[-0.8px] text-[#4c5057]">
              <p>We Craft Smooth Experinces</p>
              <p>With High Conversion.</p>
            </div>
            <div className="w-[200px] py-3">
              <Separator />
            </div>
            <p className="mt-1 max-w-[280px] text-[14px] font-medium leading-[1.45] tracking-[-0.4px] text-[#5f6673]">
              Built by <Link target="_blank" href={"https://x.com/MehulxBuilds"} className="hover:text-[var(--primary-color)] transition-all ease-in-out duration-300">@MehulxBuilds</Link>
            </p>
            ...
            <div key={group.title} aria-label={group.title}>
              ...
              <div className="mt-7 flex flex-col gap-3 font-sans text-[14px] font-medium tracking-[-0.5px] text-[#5f6673]">
                {group.links.map((link) => (
                  <Link key={link.label} href={link.href} className="transition hover:text-[var(--primary-color)]">
                    {link.label}
                  </Link>
                ))}
              </div>
          ...
          <div className="flex justify-center gap-8">
            {socialLinks.map((link) => (
              <Link
                ...
                className="text-[#9aa3af] transition hover:text-[var(--primary-color)]"
              >
      ...
      <div className="relative -mt-30 h-[280px] w-full max-w-[1530px] overflow-hidden bg-[#f8fafc] md:h-[260px] border-b border-x rounded-b-[20px]">
        <p className="absolute inset-x-0 bottom-[-72px] select-none text-center text-[170px] font-bold leading-none tracking-[-8px] text-[#d9d9d9] md:bottom-[-60px] md:text-[260px] md:tracking-[-24px]">
          STUDIOS
        </p>
